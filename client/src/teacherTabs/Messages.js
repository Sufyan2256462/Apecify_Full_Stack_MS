...
const Messages = () => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      const [sentRes, receivedRes] = await Promise.all([
        axios.get(`/api/messages/sent/${teacherData._id}/teacher`),
        axios.get(`/api/messages/received/${teacherData._id}/teacher`)
      ]);
  ...
  const getRecipientOptions = () => {
    if (recipientType === 'Teacher') {
      return recipients.teachers.filter(t => t.id !== teacherData._id);
    } else {
